...
namespace OpenAlgo
{
    ...
    public static class MessagingApi
    {
        ...
        [ExcelFunction(
            Name = "oa_telegram",
            Description = "Sends a Telegram message to one linked OpenAlgo user, matching client.telegram() in the Python SDK. By default success means queued, not delivered; pass TRUE to Wait For Delivery to attempt it immediately. Limit 30 per minute.",
            Category = MessagingCategory)]
        public static object oa_telegram(
            ...
            [ExcelArgument(Name = "Priority", Description = "Queue priority from 1 to 10. Default 5.")] object? priorityOptional = null)
        {
            ...
            return AsyncTaskUtil.RunTask(nameof(oa_telegram), new object[] { user, text, wait, (double)priority }, async () =>
            {
                var payload = new JObject
                {
                    ["username"] = user,
